...
class GestionDonnees:
    ...
    def generer_donnees(self):
        ...
        if self.modele_gen == 'lineaire':
            t_train = self.w[0] + x_train * self.w[1] + \
                np.random.randn(self.nb_train) * self.bruit
            t_test = self.w[0] + x_test * self.w[1] + \
                np.random.randn(self.nb_test) * self.bruit
        elif self.modele_gen == 'sin':
            t_train = np.sin(x_train * self.w[1] * 2) + \
                np.random.randn(self.nb_train) * self.bruit
            t_test = np.sin(x_test * self.w[1] * 2) + \
                np.random.randn(self.nb_test) * self.bruit
        else:
            t_train = np.tanh((x_train - 0.5) * self.w[1] * 2) + \
                np.random.randn(self.nb_train) * self.bruit
            t_test = np.tanh((x_test - 0.5) * self.w[1] * 2) + \
                np.random.randn(self.nb_test) * self.bruit
    ...
    def afficher_donnees_et_modele(self, x, t, scatter=True):
        ...
        t : vecteur de cibles
        scatter : variable determinant si on doit afficher une courbe ou
        des points
        """
